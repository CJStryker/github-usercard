...
*/
function GitCard(imgSrc, usersName, uName, loc, gitUrl, fsCount, fingCount, bio) {
    ...
    profileTag.href = gitUrl;
    profileTag.textContent = `Profile: ${gitUrl}`;
    followers.textContent = `Followers: ${fsCount}`;
    following.textContent = `Following: ${fingCount}`;
...
function axiosCard(cardApiUrl) {
    axios.get(cardApiUrl)
        .then((res) => {
            //console.log(res);
            const getCard = GitCard(res.data.avatar_url,
                res.data.name,
                res.data.login,
                res.data.location,
                res.data.html_url,
